...
#define GUN_GUN_JI_BET 1          // 滚滚鸡
#define JIN_YIN_JI_BET 2          // 滚滚鸡
#define XING_QI_JI_BET 1          // 星期鸡
#define JIAN_QI_WA_BET 5          // 见七挖
...
void Table::init_table_type(int set_type, int set_has_ghost, int set_has_feng, int set_hu_pair, int set_horse_num,
                            ...
                            int set_shu_zi_ji,int set_jian_qi_wa,int set_gui_yang,int set_lai_zi_ji,int set_zha_niao, int set_is_lian_zhuang)
{
    type = set_type;
    ...
    shu_zi_ji = set_shu_zi_ji;           //数字鸡
    jian_qi_wa = set_jian_qi_wa;         //见七挖
...
void Table::update_account_bet()
{
    ...
    if (xing_qi_ji == 1)
    {
        for (int cnt = 0; cnt < 3; ++cnt)
        {
            ji_data ji;
            ji.type = XING_QI_JI;
            ji.value = 16*cnt + xing_qi;
            ji_pai.push_back(ji);
            horse_count++;
            mjlog.debug("jipai xingqiji[%d]\n", ji.value);
        }
    }
    ...
    if (shu_zi_ji == 1)
    {
        ...
        { //循环每张鸡牌
            if (ji_pai[i].type == FANG_JI)
            {
                ji_data ji;
                ji.value = ji_pai[i].value;
                ji.type = SHU_ZI_JI;
                ji_pai.push_back(ji);
                horse_count++;
                mjlog.debug("jipai shuziji[%d]\n", ji.value);
                break;
            }
        }
    }
    ...
    if (jian_qi_wa == 1)
    {
        for (int i = 0; i < horse_count; i++)
        { //循环每张鸡牌
            if (ji_pai[i].value == 7 || ji_pai[i].value == 1 * 16 + 7 || ji_pai[i].value == 2 * 16 + 7)
            {
                ji_data ji;
                ji.value = ji_pai[i].value;
                ji.type = JIAN_QI_WA;
    ...
    { //根据玩家手中的鸡牌，算bet值
        ...
        for (int j = 0; j < seats[i].horse_count; ++j)
        {
            ...
            if (seats[i].ji_pai[j].value == 16 * 1 + 1 || seats[i].ji_pai[j].value == 16 * 2 + 2)
            {
                if (seats[i].ji_pai[j].type == YAO_BAI_JI || seats[i].ji_pai[j].type == FANG_JI || seats[i].ji_pai[j].type == GUN_GUN_JI)
                {
                    seats[i].ji_pai[j].type = JIN_YIN_JI;
                    mjlog.debug("jipai seats[%d] jin yin ji.\n", i);
                }
            }
            ...
            case JIN_JI:
                ...
                break;
            case JIN_YIN_JI:
                score_from_players_item_count[i][JIN_YIN_JI_TYPE]++;
                break;
            case BAO_JI:
            ...
            case GUN_GUN_JI:
                ...
                break;
            case XING_QI_JI:
                score_from_players_item_count[i][XING_QI_JI_TYPE]++;
                break;
            case SHU_ZI_JI:
                score_from_players_item_count[i][SHU_ZI_JI_TYPE]++;
                break;
            case JIAN_QI_WA:
                score_from_players_item_count[i][JIAN_QI_WA_TYPE]++;
                break;
            default:
                break;
            }
            for (int k = 0; k < seat_max; k++)
            {
                ...
                case JIN_JI:
                    ...
                    break;
                case JIN_YIN_JI:
                    seats[i].score_from_players_detail[k][JIN_YIN_JI_TYPE] += JIN_YIN_JI_BET;
                    score_to_players_item_count[k][JIN_YIN_JI_TYPE]++;
                    mjlog.debug("JIN YIN JI seatsid[%d] win %d seatsid[%d] lose\n", i, JIN_YIN_JI_BET, k);
                    break;
                case BAO_JI:
                ...
                case GUN_GUN_JI:
                    ...
                    break;
                case XING_QI_JI:
                    seats[i].score_from_players_detail[k][XING_QI_JI_TYPE] += XING_QI_JI_BET;
                    score_to_players_item_count[k][XING_QI_JI_TYPE]++;
                    mjlog.debug("XING QI JI seatsid[%d] win %d seatsid[%d] lose\n", i, XING_QI_JI_BET, k);
                    break;
                case SHU_ZI_JI:
                    seats[i].score_from_players_detail[k][SHU_ZI_JI_TYPE] += seats[i].ji_pai[j].value;
                    score_to_players_item_count[k][SHU_ZI_JI_TYPE]++;
                    mjlog.debug("SHU ZI JI seatsid[%d] win %d seatsid[%d] lose\n", i, seats[i].ji_pai[j].value, k);
                    break;
                case JIAN_QI_WA:
                    seats[i].score_from_players_detail[k][JIAN_QI_WA_TYPE] += JIAN_QI_WA_BET;
                    score_to_players_item_count[k][JIAN_QI_WA_TYPE]++;
                    mjlog.debug("JIAN QI WA seatsid[%d] win %d seatsid[%d] lose\n", i, JIAN_QI_WA_BET, k);
                    break;
                default:
                    break;
                }
    ...
    for (int i = 0; i < seat_max; i++)
    {
        ...
        for (; ite != seats[i].score_from_players_detail.end(); ite++)
        {
            ...
            if (secondValue.find(JIN_YIN_JI_TYPE) != secondValue.end())
            {
                score_from_players_item_total[i][JIN_YIN_JI_TYPE] += secondValue[JIN_YIN_JI_TYPE];
                score_to_players_item_total[firstValue][JIN_YIN_JI_TYPE] += secondValue[JIN_YIN_JI_TYPE];
                mjlog.debug("score item jin_yin_ji_type# from[%d] cnt[%d] to[%d] cnt[%d] \n", score_from_players_item_total[i][JIN_YIN_JI_TYPE], score_from_players_item_count[i][JIN_YIN_JI_TYPE],
                            score_to_players_item_total[firstValue][JIN_YIN_JI_TYPE], score_to_players_item_count[firstValue][JIN_YIN_JI_TYPE]);
            }
            ...
            if (secondValue.find(XING_QI_JI_TYPE) != secondValue.end())
            {
                score_from_players_item_total[i][XING_QI_JI_TYPE] += secondValue[XING_QI_JI_TYPE];
                score_to_players_item_total[firstValue][XING_QI_JI_TYPE] += secondValue[XING_QI_JI_TYPE];
                mjlog.debug("score item xing_qi_ji_type# from[%d] cnt[%d] to[%d] cnt[%d] \n", score_from_players_item_total[i][XING_QI_JI_TYPE], score_from_players_item_count[i][XING_QI_JI_TYPE],
                            score_to_players_item_total[firstValue][XING_QI_JI_TYPE], score_to_players_item_count[firstValue][XING_QI_JI_TYPE]);
            }
            if (secondValue.find(SHU_ZI_JI_TYPE) != secondValue.end())
            {
                score_from_players_item_total[i][SHU_ZI_JI_TYPE] += secondValue[SHU_ZI_JI_TYPE];
                score_to_players_item_total[firstValue][SHU_ZI_JI_TYPE] += secondValue[SHU_ZI_JI_TYPE];
                mjlog.debug("score item shu_zi_ji_type# from[%d] cnt[%d] to[%d] cnt[%d] \n", score_from_players_item_total[i][SHU_ZI_JI_TYPE], score_from_players_item_count[i][SHU_ZI_JI_TYPE],
                            score_to_players_item_total[firstValue][SHU_ZI_JI_TYPE], score_to_players_item_count[firstValue][SHU_ZI_JI_TYPE]);
            }
            if (secondValue.find(JIAN_QI_WA_TYPE) != secondValue.end())
            {
                score_from_players_item_total[i][JIAN_QI_WA_TYPE] += secondValue[JIAN_QI_WA_TYPE];
                score_to_players_item_total[firstValue][JIAN_QI_WA_TYPE] += secondValue[JIAN_QI_WA_TYPE];
                mjlog.debug("score item jian_qi_wa_type# from[%d] cnt[%d] to[%d] cnt[%d] \n", score_from_players_item_total[i][JIAN_QI_WA_TYPE], score_from_players_item_count[i][JIAN_QI_WA_TYPE],
                            score_to_players_item_total[firstValue][JIAN_QI_WA_TYPE], score_to_players_item_count[firstValue][JIAN_QI_WA_TYPE]);
            }
    ...
    if (zha_niao == 1 &&  deck.horse_cards.size() > 0)
    {
        ...
        int zhong_niao_seat = win_seatid;
        for (int i = 1; i < niao_value; i++)
        {
            zhong_niao_seat = next_player_seatid_of(zhong_niao_seat);
            }
        }

        //种鸟玩家的分翻倍
        if (score_from_players_total[zhong_niao_seat] >= 0)
        {
            score_from_players_item_total[zhong_niao_seat][ZHA_NIAO_TYPE] = score_from_players_total[zhong_niao_seat];
            score_from_players_item_count[zhong_niao_seat][ZHA_NIAO_TYPE]++;
            mjlog.debug("zhong niao player[%d] jin fen player[%d] jin fen type[%d] jin fen[%d]\n", zhong_niao_seat, zhong_niao_seat, ZHA_NIAO_TYPE, score_from_players_total[zhong_niao_seat]);
        }
        else
        {
            score_to_players_item_total[zhong_niao_seat][ZHA_NIAO_TYPE] = score_from_players_total[zhong_niao_seat];
            score_to_players_item_count[zhong_niao_seat][ZHA_NIAO_TYPE]++;
            mjlog.debug("zhong niao player[%d] chu fen player[%d] chu fen type[%d] chu fen[%d]\n", zhong_niao_seat, zhong_niao_seat, ZHA_NIAO_TYPE, score_from_players_total[zhong_niao_seat]);
        }
        score_from_players_total[zhong_niao_seat] *= 2;

        if ( zhong_niao_seat == win_seatid )
        { //赢家种鸟，分数都翻倍
            mjlog.debug("ying jia zhong niao!\n");
            int tmp_zhong_niao_seat = zhong_niao_seat;
            for (int i = 0; i < max_ready_players - 1; ++i)
            {
                tmp_zhong_niao_seat = next_player_seatid_of(tmp_zhong_niao_seat);
                if (score_from_players_total[tmp_zhong_niao_seat] >= 0)
                {
                    score_from_players_item_total[tmp_zhong_niao_seat][ZHA_NIAO_TYPE] = score_from_players_total[tmp_zhong_niao_seat];
                    score_from_players_item_count[tmp_zhong_niao_seat][ZHA_NIAO_TYPE]++;
                    mjlog.debug("zhong niao player[%d] jin fen player[%d] jin fen type[%d] jin fen[%d]\n", zhong_niao_seat, tmp_zhong_niao_seat, ZHA_NIAO_TYPE, score_from_players_total[tmp_zhong_niao_seat]);
                }
                else
                {
                    score_to_players_item_total[tmp_zhong_niao_seat][ZHA_NIAO_TYPE] = score_from_players_total[tmp_zhong_niao_seat];
                    score_to_players_item_count[tmp_zhong_niao_seat][ZHA_NIAO_TYPE]++;
                    mjlog.debug("zhong niao player[%d] chu fen player[%d] chu fen type[%d] chu fen[%d]\n", zhong_niao_seat, tmp_zhong_niao_seat, ZHA_NIAO_TYPE, score_from_players_total[tmp_zhong_niao_seat]);
                }
                score_from_players_total[tmp_zhong_niao_seat] *= 2;
            }
        }
        else
        {
            mjlog.debug("shu jia zhong niao!\n");
            //找到中鸟玩家多加的鸟分来源
            //中鸟玩家加的分，从其它玩家减去
            map<int, map<int, int>>::iterator ite = seats[zhong_niao_seat].score_from_players_detail.begin();
            for (; ite != seats[zhong_niao_seat].score_from_players_detail.end(); ite++)
            {
                int first_value = ite->first;
                map<int, int> second_value = ite->second;

                map<int, int>::iterator inner_ite = second_value.begin();
                for (; inner_ite != second_value.end(); inner_ite++)
                {
                    if (first_value == -1)
                    {
                        for (int k = 0; k < seat_max; k++)
                        {
                            if (seats[k].ready != 1 || k == zhong_niao_seat)
                                continue;
                            score_from_players_total[k] -= second_value[inner_ite->first] / (max_ready_players - 1);
                            score_to_players_item_total[k][ZHA_NIAO_TYPE] += second_value[inner_ite->first];
                            score_to_players_item_count[k][ZHA_NIAO_TYPE] = 1;
                            mjlog.debug("zhong niao player[%d] chu fen player[%d] chu fen type[%d] chu fen[%d]\n", zhong_niao_seat, k, inner_ite->first, second_value[inner_ite->first]);
                        }
                    }
                    else
                    {
                        score_from_players_total[first_value] -= second_value[inner_ite->first];
                        score_to_players_item_total[first_value][ZHA_NIAO_TYPE] += second_value[inner_ite->first];
                        score_to_players_item_count[first_value][ZHA_NIAO_TYPE] = 1;
                        mjlog.debug("zhong niao player[%d] chu fen player[%d] chu fen type[%d] chu fen[%d]\n", zhong_niao_seat, first_value, inner_ite->first, second_value[inner_ite->first]);
                    }
                }
            }
            ...
            for (int j = 0; j < seat_max; j++)
            {
                if (seats[j].ready != 1 || zhong_niao_seat == j)
                {
                    continue;
                }
                map<int, map<int, int>>::iterator ite = seats[j].score_from_players_detail.begin();
                for (; ite != seats[j].score_from_players_detail.end(); ite++)
                {
                    int first_value = ite->first;
                    map<int, int> second_value = ite->second;
                    ...
                    for (; inner_ite != second_value.end(); inner_ite++)
                    {
                        if (first_value == zhong_niao_seat)
                        {
                            score_from_players_total[j] += second_value[inner_ite->first];
                            score_from_players_item_count[j][ZHA_NIAO_TYPE] = 1;
                            score_from_players_item_total[j][ZHA_NIAO_TYPE] += second_value[inner_ite->first];
                            mjlog.debug("zhong niao player[%d] jin fen player[%d] jin fen type[%d] jin fen[%d]\n", zhong_niao_seat, j, inner_ite->first, second_value[inner_ite->first]);
                        }
                        else if (first_value == -1)
                        {
                            score_from_players_total[j] += second_value[inner_ite->first] / (max_ready_players - 1);
                            score_from_players_item_count[j][ZHA_NIAO_TYPE] = 1;
                            score_from_players_item_total[j][ZHA_NIAO_TYPE] += second_value[inner_ite->first] / (max_ready_players - 1);
                            mjlog.debug("zhong niao player[%d] jin fen player[%d] jin fen type[%d] jin fen[%d], max_ready_players[%d]\n", zhong_niao_seat, j, inner_ite->first, second_value[inner_ite->first] / (max_ready_players - 1), max_ready_players);
                        }
                    }
                }
            }
